...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    line = rg.Line(urcorner,llcorner)
    line.arrow = "last"
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    win.render()
    llcorner = rect.get_lower_left_corner()
    urcorner = rect.get_upper_right_corner()
    for k in range(n-1):
        corner1 = rg.Point(llcorner.x-delta*(k+1),llcorner.y+delta*(k+1))
        corner2 = rg.Point(urcorner.x+delta*(k+1),urcorner.y-delta*(k+1))
